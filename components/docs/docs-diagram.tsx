    ...
    async function renderDiagram() {
      ...
      const instance = cytoscape({
        ...
        style: [
          ...
          { selector: 'edge', style: { width: 1.4, 'line-color': '#8f826a', 'target-arrow-color': '#8f826a', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier', label: 'data(label)', color: '#655b4d', 'font-size': '9px', 'font-weight': 'bold', 'text-rotation': 'autorotate', 'text-margin-y': -9, 'text-background-color': '#fbf7e8', 'text-background-opacity': 1, 'text-background-padding': 4, 'text-background-shape': 'roundrectangle', 'text-border-color': '#d8cfb3', 'text-border-width': 1, 'text-border-opacity': 1 } },
        ],
        minZoom: 0.45,
        maxZoom: 2.3,
