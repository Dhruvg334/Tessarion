...
function confirmationMessage(reason: string | null): string {
  if (reason === 'confirmation_failed') return 'The confirmation link could not be completed. Request a new link or try signing in again.';
  if (reason === 'missing_confirmation_code') return 'The confirmation link is incomplete. Request a new confirmation email.';
  return '';
}
...
export function LoginForm() {
  ...
  const routeError = confirmationMessage(searchParams.get('reason'));
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!hasSupabaseClientEnv()) {
      setError('Supabase is not configured. Add the public Supabase URL and anonymous key, then redeploy.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError('The browser could not reach Supabase Auth. Check the public Supabase configuration and network connection.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="auth-shell">
      ...
      <section className="auth-form-wrapper">
        <div className="auth-form-card">
          ...
          {visibleError && <p className="notice" role="alert" style={{ marginBottom: '1rem' }}>{visibleError}</p>}
